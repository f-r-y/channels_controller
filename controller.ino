...
const int NB_STATE_VALUES = 16;

int states_values[NB_STATE_VALUES];

int states = 0;

const int dataPin = 5; //12;   //ser Outputs the byte to transfer
const int loadPin = 16; //13;   //rclk Controls the internal transference of data in SN74HC595 internal registers
const int clockPin = 14; //15;  //srclk Generates the clock signal to control the transference of data
//16 rclk
//14 srclk
//5 ser
...
void setup() {
  ...
  for (int i = 0; i < NB_STATE_VALUES; ++i)
  {
    states_values[i] = 0;
  }
  applyStates();
...
//SERVER INIT
  ...
  if(ssid.length()>0 && ssid!=""){
    ...
    server.on("/", []() {
      ...
      {
        int tmp = i+1;
        String arg_name = "state_";
        arg_name += tmp;
        
        int tmp_value = states_values[i];
    ...
    server.on("/updateall", []() {
      ...
      for (int i = 0; i < NB_STATE_VALUES; ++i)
      {
        ...
        int tmp_value = server.arg(arg_name).toInt();
        
        states_values[i] = tmp_value;
    ...
    server.on("/updateone", []() {
      ...
      Serial.println("m-à-j unitaire");
      
      int i = server.arg("state_id").toInt();
      int tmp_value = server.arg("value").toInt();
      
      Serial.println(i-1);
      Serial.println(tmp_value);
      
      states_values[i-1] = tmp_value;
...
void applyStates() {
  ...
  for (int i = 0; i < NB_STATE_VALUES; ++i)
  {
//    Serial.println("apply states:");
//    Serial.print("state");
//    Serial.print(i);
//    Serial.print(" = ");
//    Serial.println(states_values[i]);
    states += int(pow(2,i))*states_values[i];//tmp_value = 0 ou 1
  }
  
//    Serial.print("total : ");
//    Serial.println(states);
  
   digitalWrite(loadPin, LOW); //Disable any internal transference in the SN74HC595
   shiftOut(dataPin, clockPin, MSBFIRST, states); //Transfer data to the SN74HC595
   digitalWrite(loadPin, HIGH); //Start the internal data transference in the SN74HC595
}
...
//    for (int i= 0; i<=7; i++){
//       digitalWrite(loadPin, LOW); //Disable any internal transference in the SN74HC595
//       shiftOut(dataPin, clockPin, MSBFIRST, i); //Transfer data to the SN74HC595
//       digitalWrite(loadPin, HIGH); //Start the internal data transference in the SN74HC595
